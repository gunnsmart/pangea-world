...
class Brain:
    ...
    def _compute_scores(self, pain: dict, perc: dict) -> dict[str, float]:
        ...
        for action in ACTIONS:
            ...
            if not feasible:
                scores[action] = 0.001  # ทำไม่ได้ แต่ไม่ใช่ศูนย์ (curiosity)
                continue
    ...
    def receive_pain(self, source: str, intensity: float):
        """
        ร่างกายส่ง pain — สมองบันทึกว่า action ล่าสุดทำให้เจ็บ
        """
        self.learn(self.current_action, -intensity, f"pain:{source}")
        drive_map = {
            "hunger": "hunger", "cold": "cold",
            "injury": "fear", "disease": "fear",
        }
        if source in drive_map:
            self.drives.fear = min(100, self.drives.fear + intensity * 10)
