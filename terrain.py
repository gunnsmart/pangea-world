...
# ─── Biome IDs ───────────────────────────────────────────────
DEEP_WATER = 0   # 🌊 ทะเลสาบ/แม่น้ำลึก
SHALLOW    = 1   # 🏖 ริมน้ำตื้น
BEACH      = 2   # 🏝 ชายหาด
GRASSLAND  = 3   # 🌿 ทุ่งหญ้า
TROPICAL   = 4   # 🌴 เขตร้อนชื้น
FOREST     = 5   # 🌳 ป่าทึบ
MOUNTAIN   = 6   # ⛰ ภูเขา
PEAK       = 7   # 🏔 ยอดเขา/ถ้ำ
...
def _island_mask(size):
    """สร้าง mask เกาะ — น้ำล้อมรอบ ใช้ ellipse gradient"""
...
def _generate_template(size, seed=42):
    ...
    for r in range(size):
        for c in range(size):
            h   = hmap[r][c] * island[r][c]   # ปรับ height ด้วย island mask
            lat = abs(r - size/2) / (size/2)
            ...
            if h < 0.05:
                ...
                template[r][c] = TROPICAL if lat < 0.3 else GRASSLAND
            elif h < 0.55:
                template[r][c] = FOREST
            elif h < 0.70:
                template[r][c] = MOUNTAIN
            else:
                template[r][c] = PEAK

    # แม่น้ำและทะเลสาบกลางเกาะ
    template = _place_lake(template, size, seed=seed+5)
    template = _carve_river(template, size, seed=seed+7)
    return template
...
class TerrainMap:
    ...
    def __init__(self, seed=42):
        self.size    = SIZE
        self.seed    = seed
        self.mapping = BIOME_NAME
        self.colors  = BIOME_COLOR
        self.template   = _generate_template(SIZE, seed)
        self.vegetation = [
            [self._init_veg(r, c) for c in range(SIZE)]
            for r in range(SIZE)
        ]
