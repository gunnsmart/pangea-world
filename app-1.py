...
with tab_humans:
    ...
    for i, h in enumerate(humans):
        ...
        with col:
            ...
            # ── Brain ──
            st.divider()
            st.caption("**🧠 Brain & Learning**")
            br = h.brain.summary
            bc1,bc2,bc3 = st.columns(3)
            bc1.metric('🔥 ทักษะไฟ',   br['skill_fire'])
            bc2.metric('🔨 ทักษะสร้าง', br['skill_craft'])
            bc3.metric('🏹 ทักษะล่า',   br['skill_hunt'])
            st.caption(f"🎯 Action: **{br['action']}** | 🧠 ความทรงจำ: {br['memories']}")
            known = []
            if br['knows_fire']: known.append('🔥 จุดไฟ')
            if br['knows_cook']: known.append('🍖 ปรุงอาหาร')
            if known: st.caption('✅ รู้จัก: ' + ', '.join(known))
            st.caption('📋 Top actions: ' + ', '.join(f'{a}({w:.1f})' for a,w in br['top_actions']))
