...
SIM_SECONDS_PER_REAL_SECOND = 1800  # 1 วินาที = 30 นาที sim
SIM_STEP_INTERVAL = 2.0             # step ทุก 2 วินาที = 1 ชั่วโมง sim
...
# ══════════════════════════════════════════════════════════════════════════════
def run_simulation():
    """Background thread — step world ทุก SIM_STEP_INTERVAL วินาที"""
    while True:
...
def _execute_action(h, partner, action, perc, info_now, near_fire,
    ...
    if action == "sleep":
        ...
        if not ate and info_now["food_level"] > 10:
            sim.terrain.vegetation[h.pos[0]][h.pos[1]] = max(0, info_now["food_level"]-10)
