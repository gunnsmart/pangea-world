...
// 8-bit Isometric Pixel Art Edition with Session Support
let ws = null;
let canvas = null;
let ctx = null;
let lastState = null;
...
function drawHumanSprite(ctx, x, y, size, sex, name) {
    ...
    ctx.beginPath(); ctx.ellipse(x, y, size/2, size/4, 0, 0, Math.PI * 2); ctx.fill();
    ...
    if (sex === 'M') ctx.fillRect(x - 3*s, y - 17*s, 6*s, 2*s);
    else { ctx.fillRect(x - 4*s, y - 17*s, 8*s, 3*s); ctx.fillRect(x - 4*s, y - 14*s, 1*s, 4*s); ctx.fillRect(x + 3*s, y - 14*s, 1*s, 4*s); }
    ctx.fillStyle = '#000'; ctx.fillRect(x - 1.5*s, y - 13*s, 1*s, 1*s); ctx.fillRect(x + 0.5*s, y - 13*s, 1*s, 1*s);
    ctx.font = `bold 10px monospace`; ctx.textAlign = 'center'; ctx.fillStyle = 'white'; ctx.fillText(name, x, y - 20*s);
}

function drawTreeSprite(ctx, x, y, size) {
    const s = size / 16;
    ctx.fillStyle = '#5d4037'; ctx.fillRect(x - 2*s, y - 4*s, 4*s, 6*s);
    ctx.fillStyle = '#2e7d32'; ctx.beginPath(); ctx.moveTo(x, y - 16*s); ctx.lineTo(x - 8*s, y - 4*s); ctx.lineTo(x + 8*s, y - 4*s); ctx.fill();
    ctx.fillStyle = '#388e3c'; ctx.beginPath(); ctx.moveTo(x, y - 22*s); ctx.lineTo(x - 6*s, y - 10*s); ctx.lineTo(x + 6*s, y - 10*s); ctx.fill();
}

function drawFireSprite(ctx, x, y, size, frame) {
    const s = size / 16;
    ctx.fillStyle = '#3e2723'; ctx.fillRect(x - 4*s, y - 1*s, 8*s, 2*s);
    const offset = Math.sin(frame * 0.2) * 2;
    ctx.fillStyle = '#ffab00'; ctx.beginPath(); ctx.moveTo(x, y - (10+offset)*s); ctx.lineTo(x - 5*s, y - 2*s); ctx.lineTo(x + 5*s, y - 2*s); ctx.fill();
}

function drawShelterSprite(ctx, x, y, size) {
    const s = size / 16;
    ctx.fillStyle = '#795548'; ctx.beginPath(); ctx.moveTo(x, y - 12*s); ctx.lineTo(x - 10*s, y + 2*s); ctx.lineTo(x + 10*s, y + 2*s); ctx.closePath(); ctx.fill();
}
...
function initWebSocket(sid) {
    ...
    ws.onclose = () => setTimeout(() => initWebSocket(sessionId), 3000);
}

// ---------- UI Rendering ----------
function getBarColor(val, inverse = false) {
    if (inverse) { if (val > 70) return '#ff3b30'; if (val > 40) return '#ffcc00'; return '#4cd964'; }
    if (val > 70) return '#4cd964'; if (val > 40) return '#ffcc00'; return '#ff3b30';
}
...
function renderFull(state) {
    ...
    if (state.game_over) {
        ...
        document.getElementById('day-display').innerText = `Day ${state.day} - GAME OVER`;
    } else {
    ...
    if (cameraFocus !== 'world') {
        ...
        if (target) {
            targetCameraPos = { x: Array.isArray(target.pos) ? target.pos[1] : target.pos.x, y: Array.isArray(target.pos) ? target.pos[0] : target.pos.y };
            targetZoom = 2.0;
        }
    } else { targetCameraPos = { x: 50, y: 50 }; targetZoom = 1.0; }
}

function renderHumans(humans) {
    const container = document.getElementById('humans-container');
    if (!humans || !humans.length) { container.innerHTML = '<div class="stat-card">ไม่มีมนุษย์</div>'; return; }
        ...
        return `
            <div class="human-card">
                ...
                <div class="human-stats-grid" style="margin-top:15px; text-align:center">
                    <div class="human-stat" style="flex-direction:column"><span style="color:#e74c3c; font-size:9px">CORTISOL</span><strong style="font-size:14px">${cortisol}</strong></div>
                    <div class="human-stat" style="flex-direction:column"><span style="color:#2ecc71; font-size:9px">OXYTOCIN</span><strong style="font-size:14px">${oxytocin}</strong></div>
                    <div class="human-stat" style="flex-direction:column"><span style="color:#f1c40f; font-size:9px">ATP</span><strong style="font-size:14px">${atp}</strong></div>
                </div>
                <div style="font-size:10px; color:#888; margin-top:10px; border-top:1px solid #1e2d3d; padding-top:8px;">🎒 กระเป๋า: ${h.inventory.join(', ') || 'ว่างเปล่า'}</div>
...
function addLog(msg) {
    ...
    entry.className = 'log-entry'; entry.textContent = msg; container.prepend(entry);
...
function drawLoop() {
    ...
    if (!canvas) { requestAnimationFrame(drawLoop); return; }
    if (!ctx) ctx = canvas.getContext('2d');
    if (!lastState || !lastState.map) { requestAnimationFrame(drawLoop); return; }

    cameraPos.x += (targetCameraPos.x - cameraPos.x) * 0.1;
    cameraPos.y += (targetCameraPos.y - cameraPos.y) * 0.1;
    ...
    for (let x = startX; x < endX; x++) {
        for (let y = startY; y < endY; y++) {
            ...
            ctx.beginPath(); ctx.moveTo(iso.x, iso.y); ctx.lineTo(iso.x + TILE_WIDTH/2, iso.y + TILE_HEIGHT/2); ctx.lineTo(iso.x, iso.y + TILE_HEIGHT); ctx.lineTo(iso.x - TILE_WIDTH/2, iso.y + TILE_HEIGHT/2); ctx.closePath(); ctx.fill();
            if (g > 100 && g > r && g > b && (x+y) % 7 === 0) drawTreeSprite(ctx, iso.x, iso.y + TILE_HEIGHT/2, 16);
        }
    }
    ...
    if (lastState.humans) {
        for (const h of lastState.humans) {
            ...
            if (h.last_speech) {
                ctx.font = '8px monospace'; const tw = ctx.measureText(h.last_speech).width;
                ctx.fillStyle = 'rgba(0,0,0,0.8)'; ctx.fillRect(iso.x - tw/2 - 2, iso.y - 30, tw + 4, 10);
                ctx.fillStyle = '#39ff14'; ctx.fillText(h.last_speech, iso.x - tw/2, iso.y - 22);
            }
        }
    }
    ctx.restore();
...
function sendCmd(cmd) {
    if (!sessionId) return;
    fetch(`/api/command/${sessionId}/${cmd}`, { method: 'POST' }).catch(e => console.error(e));
}
