...
// 8-bit Isometric Pixel Art Edition with Game Over handling
...
function renderFull(state) {
    ...
    // ✅ จัดการ game over state
    const gameOverBanner = document.getElementById('game-over-banner');
    if (state.game_over) {
        if (gameOverBanner) {
            gameOverBanner.classList.add('show');
        }
        // ยังแสดงข้อมูลพื้นฐานแม้เกมจบ
...
function drawLoop() {
    ...
    // ✅ Draw Fires (แก้ fire_spots → fires)
    if (lastState.fires && lastState.fires.length > 0) {
        for (const f of lastState.fires) {
            const iso = cartesianToIso(f.pos[1], f.pos[0]);
